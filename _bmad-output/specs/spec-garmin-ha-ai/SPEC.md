...
- **CAP-1**
  - **intent:** User can authenticate Garmin Connect credentials via UI Config Flow (with MFA PIN verification step) so that Home Assistant securely obtains and persists OAuth tokens in local storage without keeping raw passwords.
  - **success:** Config Flow completes `step_mfa`, saves tokens to `.storage/garmin_ha_ai_tokens.json`, and silently refreshes expired tokens or triggers `ConfigEntryAuthFailed` on invalid sessions.

- **CAP-2**
  - **intent:** Integration automatically extracts daily fitness and health metrics from Garmin Connect for a specified date so that health statistics are normalized for LLM prompt assembly and entity updates.
  - **success:** Ingests steps, distance, calories, resting HR, average stress, sleep score, body battery min/max, HRV status, weight, and logged activities into a structured `GarminDailyMetrics` object.
...
- **CAP-3**
  ...
  - **success:** Scheduled timer triggers coordinator sync at the exact configured schedule (default 06:00 AM), and calling `garmin_ha_ai.generate_report` executes an immediate manual refresh.

- **CAP-4**
  - **intent:** System sends structured prompt payloads combining current metrics, 7-day trend history, user goals, and directives to the active AI provider (Google Gemini or OpenAI-compatible endpoint) so that personal health summaries and workout advice are generated.
  - **success:** Valid API key returns a structured `AIHealthReport` containing both a concise 1-2 sentence dashboard summary and a full Markdown report.

- **CAP-5**
  - **intent:** System stores daily metric snapshots locally in Home Assistant `.storage/garmin_ha_ai_history.json` with a user-configurable retention window (default 30 days) so that historical trend context is available offline without hitting Garmin cloud rate limits.
  - **success:** Snapshots up to N days are preserved locally and pruned automatically, enabling instant 7-day or 30-day context assembly during AI queries.
...
- **CAP-6**
  ...
  - **success:** Numeric sensors (`sensor.garmin_steps`, `sensor.garmin_sleep_score`, etc.) render valid states, `sensor.garmin_ai_health_report_short` state stays under 255 chars, and `sensor.garmin_ai_health_report_long` stores the full Markdown report in `extra_state_attributes["full_report"]`.
...
- **CAP-7**
  ...
  - **success:** Report generation executes a call to configured `notify` services with short and long report payloads.

- **CAP-8**
  - **intent:** User can submit custom health or workout questions via service `garmin_ha_ai.ask_question` with automatic 7-day metric grounding so that personalized AI answers are returned directly in service response data and updated on `sensor.garmin_ai_last_answer`.
  - **success:** Service call returns `{"answer": "...", "question": "..."}` as direct service response data (`SupportsResponse.OPTIONAL`) and updates `sensor.garmin_ai_last_answer` within <10 seconds.
...
- MUST support Google Gemini API via `google-genai` SDK and generic OpenAI-compatible `/v1/chat/completions` API via `httpx` async client.
- MUST protect entity state strings from exceeding Home Assistant's 255-character hard limit by placing full rich Markdown reports in `extra_state_attributes["full_report"]`.
- Service `garmin_ha_ai.ask_question` MUST use `supports_response=SupportsResponse.OPTIONAL` and pull historical metrics from local `.storage` cache to prevent Garmin API rate limiting.
